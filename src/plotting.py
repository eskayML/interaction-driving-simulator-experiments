...
def plot_speaker_charts(df):
    ...
    word_counts_for_roles = df.groupby("speaker_id")["word_count"].sum().sort_values(ascending=False)
    ...
    sns.barplot(
        data=word_counts, y="speaker_id", x="word_count", ax=ax1, orient="h", palette=speaker_color_map
    )
    ...
    # 3. Sentiment Distribution (Grouped Bar - Horizontal)
    sentiment_colors = {"Positive": "green", "Negative": "red", "Neutral": "gray"}
    
    sentiment_order = ['Negative', 'Neutral', 'Positive']
    existing_sentiments = [s for s in sentiment_order if s in sentiment_distribution.columns]
    ...
    sns.barplot(
        data=intensity_scores, y="speaker_id", x="intensity", ax=ax4, orient="h", palette=speaker_color_map
    )
    ...
    legend_patches = [plt.Rectangle((0, 0), 1, 1, color=speaker_color_map[speaker]) for speaker in unique_speakers]
    ax6.legend(legend_patches, legend_labels, loc='center', frameon=True, edgecolor='black', fontsize=12)

    plt.subplots_adjust(top=0.95, bottom=0.05, left=0.1, right=0.95, hspace=0.8, wspace=0.3)
